...
// API에서 반환되는 데이터가 올바른지 확인
fetchAllPosts().then(data => console.log(data));
...
// 댓글 생성
export const createComment = async commentCreateRequestDto => {
  try {
    const response = await axiosWithToken.post(
      `/api/comments`,
...
// 댓글 수정
export const updateComment = async (commentId, commentUpdateRequestDto) => {
  try {
    const response = await axiosWithToken.put(
